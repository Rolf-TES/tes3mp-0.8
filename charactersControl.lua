...
customEventHooks.registerValidator("OnPlayerLevel", function(eventStatus, pid)
    if tes3mp.GetLevel(pid) > maxLevel and Players[pid].data.stats.level > maxLevel and Players[pid] ~= nil and Players[pid]:IsLoggedIn() then
        Players[pid].data.stats.level = maxLevel
        Players[pid].data.stats.levelProgress = 0
        Players[pid]:LoadLevel(Players[pid].pid)
    end
end)
customEventHooks.registerValidator("OnPlayerCellChange", function(eventStatus, pid)
    if Players[pid] ~= nil and Players[pid]:IsLoggedIn() then
        ...
        Players[pid]:LoadStatsDynamic(Players[pid].pid)
    end
end)
customEventHooks.registerValidator("OnPlayerAttribute", function(eventStatus, pid)
    if Players[pid] ~= nil and Players[pid]:IsLoggedIn() then
        ...
        Players[pid]:LoadAttributes(Players[pid].pid)
    end
end)
customEventHooks.registerValidator("OnPlayerSkill", function(eventStatus, pid)
    if Players[pid] ~= nil and Players[pid]:IsLoggedIn() then
        ...
        Players[pid]:LoadSkills(players[pid].pid)
    end
end)